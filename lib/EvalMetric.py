...
def cal_metrics_train(data_type, pre_path, GT_path):
    ...
    for i, image_file in enumerate(image_files):
        ...
        single_TP = np.sum(img*lab)
        single_sum_areas = np.sum(img) + np.sum(lab)
        if single_sum_areas == 0:
            single_dice = 1.0 if single_TP == 0 else 0.0
        else:
            single_dice = (2.0 * single_TP) / single_sum_areas
        Dice_.append(single_dice)
